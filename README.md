...
|---|---:|---:|---:|---:|---:|
| MMLU-Pro | 42 | 0.8095 | 0.9048 | **0.8810** | −2.4pp |
| IFEval | 20 / 19 | 0.7000 | 0.9474 | **0.8947** | −5.3pp |
| AIME24 | 10 | 0.3000 | 0.8000 | **1.0000** | **+20.0pp** |
| C-EVAL | 20 | 0.8500 | 0.9500 | **0.9500** | 0.0 |
| LiveCodeBench | 10 | 0.7000 | 1.0000 | **1.0000** | 0.0 |
| **综合（等权）** | 102 | **67.2** | **92.0** | **94.5** | **+2.5** |

**根因不是模型变聪明，而是 `max_tokens` 截断**：第一轮有 **22/102** 道题的推理链被砍断（AIME24 高达 7/10），答案字段为空自然判 0。四轮截断题数 **22 → 3 → 1 → 0**，综合分 **67.2 → 92.0 → 94.5**。这也再次印证第七节第 8 条：`reasoning_effort` 合法值只有 **xhigh / medium / low**，没有 `high`。
