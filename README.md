...
**AIME24 追测（第三轮，只重跑这一项）**：再把 `max_tokens` 从 32768 提到 **65536**：
...
放宽上限确实又拿回一题——但剩下那 1 题消耗了 **65528 个推理 token** 仍顶满上限、没吐出答案。AIME24 在 xhigh 下的推理长度是**长尾分布**（其余九题 663～32917 token，中位数仅 ~9.4K），**调大 `max_tokens` 只能收敛截断、无法彻底消除**。若回填 0.90，综合分 **92.0 → 94.0**。
