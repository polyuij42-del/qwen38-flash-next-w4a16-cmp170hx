...
## 九、能力评测：LLST 102 题标准体检（四轮：截断版 / xhigh / AIME 追测 / 不限制）

速度之外，本机也跑了一遍 [Local LLM Standard Test (LLST) v1.0](https://github.com/yang2020chen/local-llm-standard-test)——**102 道锁定题目**（MMLU-Pro 42 / IFEval 20 / AIME24 10 / C-EVAL 20 / LiveCodeBench 10）+ 4 档长上下文性能（512 / 4096 / 16384 / 28672）。同一套锁定题库跑了四轮，**变量只有思考强度与生成上限 `max_tokens`**：

| 子项 | 题量 | R1 截断版 | R2 xhigh | R3 仅 AIME | **R4 不限制** | R4 vs R2 |
|---|---:|---:|---:|---:|---:|---:|
| MMLU-Pro | 42 | 0.8095 | 0.9048 | ＝R2 | **0.8810** | −2.4pp |
| IFEval | 20 / 19 | 0.7000 | 0.9474 | ＝R2 | **0.8947** | −5.3pp |
| AIME24 | 10 | 0.3000 | 0.8000 | **0.9000** | **1.0000** | **+20.0pp** |
| C-EVAL | 20 | 0.8500 | 0.9500 | ＝R2 | **0.9500** | 0.0 |
| LiveCodeBench | 10 | 0.7000 | 1.0000 | ＝R2 | **1.0000** | 0.0 |
| **综合（等权）** | 102 | **67.2** | **92.0** | **94.0（回填）** | **94.5** | **+2.5** |

> **第三轮是单基准重跑**：只重跑了 AIME24（上限 32768 → 65536，10 题），其余四项**未执行**，表中以 `＝R2` 标注。综合分一栏的 **94.0** 是"R2 四项 + R3 AIME24"的回填值，用于把四轮串成一条可比曲线，不代表第三轮跑过全量 102 题。

**根因不是模型变聪明，而是 `max_tokens` 截断**：第一轮有 **22/102** 道题的推理链被砍断（AIME24 高达 7/10），答案字段为空自然判 0。四轮截断题数 **22 → 3 → 1 → 0**（第三轮的分母是 AIME24 的 10 题），综合分 **67.2 → 92.0 → 94.0（回填）→ 94.5**。这也再次印证第七节第 8 条：`reasoning_effort` 合法值只有 **xhigh / medium / low**，没有 `high`。

**AIME24 上限梯度（四轮，唯一真正被上限左右的基准）**：

| AIME24 | `max_tokens` | 得分 | 截断 |
|---|---:|---:|---:|
| 第一轮 | 偏小 | 0.30 | 7 / 10 |
| 第二轮 | 32768 | 0.80 | 2 / 10 |
| 第三轮（仅重跑此项） | 65536 | 0.90 | 1 / 10 |
| **第四轮（全量不限制）** | **250000** | **1.00** | **0** |

**那道反复失败的第 4 题（目标答案 385）最终需要 94,579 个推理 token**——在 32768 和 65536 上限下都被砍断、判 0，上限放到 250000 后一次性写满并答对。**它不是"停不下来"，只是真的要思考近 10 万 token。** 该题独占 AIME24 推理总量的 47%，十题分布最小 552 / 中位 8,438 / 最大 94,579 ⇒ **典型长尾**。

反过来说，**放开上限对那些本来就没截断的基准没有增益**：MMLU-Pro 与 IFEval 各有 1 道题判反（且 R4 两项都是零截断），属单轮采样波动（服务端强制 `temperature=1.0`）；而同两项的推理 token 总量反而涨了 **+65% / +48%**——**想得更多 ≠ 答得更对**。

⚠️ **速度指标准读法**：LLST 报表的 `output_throughput_tps` 是**端到端平均**（输出 token ÷（TTFT + 解码）），512 档显示 108.6 而非监控上的 ~147 tok/s。**真实单请求解码速度 = 1/TPOT**，四轮实测 55~125 tok/s。性能段每档只有 2 个请求且紧跟在长时间能力测试之后，**单轮噪声很大，不适合当结论**；第一节的 104 tok/s 单流解码是空载专项测试。

完整四轮报告（含长图、自包含仪表盘、逐题截断统计、逐题推理 token、原始汇总）：**[`results/llst/`](results/llst/README.md)**

```bash
# 复现（Mac 端一键驱动，也可直接调 remote 上的 scripts/run_standard_test.sh）
llst run --force                          # 全量 102 题（默认 xhigh 协议）
LLST_PROTOCOL=standard_test_v1.unlimited.yaml llst run --force   # 不限制 token
...
*实测平台：msi · Ubuntu 26.04 · kernel 7.0.0-30 · 2× CMP 170HX 64GB · vLLM fork。性能数据日期 2026-09-15 ~ 09-16；LLST 能力数据日期 2026-09-18（四轮）。*
