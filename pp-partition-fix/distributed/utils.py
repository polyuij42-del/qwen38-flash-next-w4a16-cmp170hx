...
def get_pp_indices(
    ...
    if partition_list_str is not None:
        ...
        if len(partitions) != pp_size:
            # PLE-offload/PP patch: sidecar processes (e.g. PleOffloadWorker)
            # run with their own pp_size=1 but inherit the
            # VLLM_PP_LAYER_PARTITION meant for the main PP ranks. Fall back
            # to the default even split instead of hard-failing.
            logger.warning(
                "VLLM_PP_LAYER_PARTITION %s does not match pp_size=%d; "
                "falling back to default layer partitioning.",
                partition_list_str,
                pp_size,
            )
            partitions = None
        if partitions is not None and sum(partitions) != num_hidden_layers:
